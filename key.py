...
def parse(page: soup) -> dict:
    ...
    requests = [get_requests(block) for block in page.find_all('div', attrs={'class':'question-options'})]
...
def get_answers(page: soup) -> list:
    return [e.find('div', attrs={'class':'col-md-11 no-padding'}).p.text for e in page.find_all('div', attrs={'class':'entry-item question-view-item'})]
...
def get_requests(block: soup) -> dict:
    ...
    for e in block.find_all('div', attrs={'class':'text-only-option'}):
        ...
        result.update({letter: e.find('div', attrs={'class':'option-text'}).p.text})
    return result
