...
class Find:
    def __init__(self, test_name: str, amount: int, subject: str, klass: int):
        self.name = name
        self.amount = amount
        code = requests.get(f'https://naurok.com.ua/test/{subject}/klas-{klass}?q={test_name}').content
        page = soup(code, 'html.parser')
        self.blocks = page.find_all('div', attrs={'class':'file-item test-item'})
    ...
    def filter_blocks(self, block: soup) -> soup:
        ...
        if int(block.find('div', attrs={'class':'testCounter'}).text) == self.amount and link.text == self.name:
